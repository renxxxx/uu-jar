...
public class Jdbcuu2 {
	private static Logger logger = LoggerFactory.getLogger(Jdbcuu2.class);
	...
	public static JSONArray rows(Connection conn, String sql, List params) throws Exception {
		return rows(conn, sql, params.toArray());
	}

	public static JSONArray rows(Connection conn, String sql, Object... params) throws Exception {
...
//	public static JSONArray thinRows(Connection conn, String sql, Object... params) throws Exception {
//		PreparedStatement pst = null;
//		try {
//			pst = conn.prepareStatement(sql);
...
//	}
	...
	public static JSONArray getList(Connection conn, String sql, Object... params) throws Exception {
		return JSON.parseArray(getString(conn, sql, params));
	}

	public static JSONObject getMap(Connection conn, String sql, Object... params) throws Exception {
		return JSON.parseObject(getString(conn, sql, params));
	}
	...
	public static Object getObject(Connection conn, String sql, Object... params) throws Exception {
		JSONObject row = row(conn, sql, params);
		if (row == null)
			return null;
...
//	}
	...
	public static JSONObject row(Connection conn, String sql, JSONArray params) throws Exception {
		return row(conn, sql, params.toArray());
	}

	public static JSONObject row(Connection conn, String sql, List params) throws Exception {
		return row(conn, sql, params.toArray());
	}

	public static JSONObject row(Connection conn, String sql, Object... params) throws Exception {

		JSONObject item = null;
		JSONArray itemList = rows(conn, sql, params);
...
//	public static JSONArray rowsCommonly(Connection conn, String table, JSONArray columns, JSONObject conditionm,
//			JSONObject orderm, JSONArray limits) throws Exception {
//		String sql = "";
//		sql += "select ";
//
//		if (columns.isEmpty()) {
...
//		if (conditionm != null && !conditionm.isEmpty()) {
//			sql += " where ";
//			for (Iterator iterator = conditionm.keySet().iterator(); iterator.hasNext();) {
...
//		}
//
//		if (orderm != null && !orderm.isEmpty()) {
//			sql += " order by ";
//			for (Iterator iterator = orderm.keySet().iterator(); iterator.hasNext();) {
//				Object key = (Object) iterator.next();
...
//	}
	...
	public static int update(Connection conn, String table, JSONObject columnm, JSONObject conditionm)
		...
		JSONArray params = new JSONArray();
		if (columnm != null) {
			for (Iterator iterator = columnm.keySet().iterator(); iterator.hasNext();) {
				Object key = (Object) iterator.next();
...
//	public static int insertByCustomKeyCommonly(Connection conn, String table, JSONObject columnm) throws Exception {
//		String sql = "";
//		sql += "insert into ";
//		sql += "`" + table + "`";
//		sql += " ( ";
...
//	}
	...
	public static int insert(Connection conn, String table, Object columnm, Object value) throws Exception {
		JSONObject conditionm = new JSONObject();
		conditionm.put((String) columnm, value);
		return insert(conn, table, conditionm);
	}

	public static int insert(Connection conn, String table, Object[] columnms, Object[] values) throws Exception {
		JSONObject columnm = new JSONObject();
		columnms = columnms == null ? new Object[] {} : columnms;
		values = values == null ? new Object[] {} : values;
		for (int i = 0; i < columnms.length; i++) {
	...
	public static int insert(Connection conn, String table, JSONObject columnm) throws Exception {
		String sql = "";
		sql += "insert into ";
		sql += "`" + table + "`";
		sql += " ( ";
	...
	public static Integer getGeneratedKey(PreparedStatement pst) throws SQLException {
		ResultSet rs = pst.getGeneratedKeys();
		if (rs.next()) {
			int id = rs.getInt(1);
			rs.close();
	...
	public static List<Integer> getGeneratedKeys(PreparedStatement pst) throws SQLException {
		ResultSet rs = pst.getGeneratedKeys();
...
//	}
	...
	public static JSONArray rows(ResultSet rs) throws SQLException {
...
//		int columnCnt = metaData.getColumnCount();
//
//		JSONArray<List<Object>> valueLists = new ArrayList<List<Object>>();
//		while (rs.next()) {
...
//	public static JSONArray thinRows(ResultSet rs) throws SQLException {
//		JSONArray rows = new JSONArray();
//		ResultSetMetaData metaData = rs.getMetaData();
//		while (rs.next()) {
...
//	}
	...
	public static JSONObject row(ResultSet rs) throws SQLException {
		JSONArray rows = rows(rs);
		if (rows.size() > 0)
	...
	public static Object getObject(ResultSet rs) throws SQLException {
		JSONArray rows = rows(rs);
		if (rows.size() > 0)
			return rows.getJSONObject(0).get(rows.getJSONObject(0).keySet().iterator().next());
		else
			return null;
	}

	public static Integer getInteger(ResultSet rs) throws SQLException {
		Object value = getObject(rs);
		if (value == null)
			return null;
		else {
