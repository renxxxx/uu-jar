...
public class Param {
	...
	public Param vNull() {
		if (!this.todo)
			return this;
		if (this.value == null)
			throw ModuleBreak.todo(1001, "\"" + this.name + "\"不能空").setErrParam(this.code);
		return this;
	}
	...
	public Param vLen(int length) {
		...
		if (this.value != null && !this.value.isEmpty() && length > -1 && this.value.length() != length) {
			throw ModuleBreak.todo(1001, "\"" + this.name + "\"长度只能是" + length).setErrParam(this.code);
		}
		return this;
	}
	...
	public Param vMinLen(int length) {
		if (!this.todo)
			return this;
		if (this.value != null && !this.value.isEmpty() && length > -1 && this.value.length() < length) {
			throw ModuleBreak.todo(1001, "\"" + this.name + "\"长度最低" + length).setErrParam(this.code);
		}
		return this;
	}
	...
	public Param vMaxLen(int length) {
		if (!this.todo)
			return this;
		if (this.value != null && !this.value.isEmpty() && length > -1 && this.value.length() > length) {
			throw ModuleBreak.todo(1001, "\"" + this.name + "\"长度最大" + length).setErrParam(this.code);
		}
		return this;
	}
	...
	public Param vMaxNum(float maxnum) {
		...
		if (!isEmpty() && toFloat() > maxnum) {
			throw ModuleBreak.todo(1001, "\"" + this.name + "\"最大" + maxnum).setErrParam(this.code);
		}
		return this;
	}
	...
	public Param vMinNum(float minnum) {
		...
		if (!isEmpty() && toFloat() < minnum) {
			throw ModuleBreak.todo(1001, "\"" + this.name + "\"最小" + minnum).setErrParam(this.code);
		}
		return this;
	}
	...
	public Param vReg(Pattern regex, String note) {
		...
		if (this.value != null && !this.value.isEmpty() && regex != null) {
			if (!regex.matcher(this.value).matches())
				throw ModuleBreak
						.todo(1001, "\"" + this.name + "\"有误" + (note == null || note.isEmpty() ? "" : ",要求：" + note))
	...
	public void bomb(String message) {
		throw ModuleBreak.todo(1001, "\"" + this.name + "\"" + message).setErrParam(this.code);
	}
