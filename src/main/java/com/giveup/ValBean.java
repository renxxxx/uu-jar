...
public class ValBean {
	...
	public ValBean vNull() {
		if (this.value == null)
			throw CellReturn.failure(1001, "\"" + this.name + "\"不能空").setErrParam(this.code);
		return this;
	}
	...
	public ValBean vLen(int length) {
		if (this.value != null && !this.value.isEmpty() && length > -1 && this.value.length() != length) {
			throw CellReturn.failure(1001, "\"" + this.name + "\"长度只能是" + length).setErrParam(this.code);
		}
		return this;
	}
	...
	public ValBean vMaxLen(int length) {
		if (this.value != null && !this.value.isEmpty() && length > -1 && this.value.length() > length) {
			throw CellReturn.failure(1001, "\"" + this.name + "\"长度最大" + length).setErrParam(this.code);
		}
		return this;
	}
	...
	public ValBean vMaxNum(float maxnum) {
		if (!isEmpty() && toFloat() > maxnum) {
			throw CellReturn.failure(1001, "\"" + this.name + "\"最大" + maxnum).setErrParam(this.code);
		}
		return this;
	}
	...
	public ValBean vMinNum(float minnum) {
		if (!isEmpty() && toFloat() < minnum) {
			throw CellReturn.failure(1001, "\"" + this.name + "\"最小" + minnum).setErrParam(this.code);
		}
		return this;
	}
	...
	public ValBean vMinLen(int length) {
		if (this.value != null && !this.value.isEmpty() && length > -1 && this.value.length() < length) {
			throw CellReturn.failure(1001, "\"" + this.name + "\"长度最低" + length).setErrParam(this.code);
		}
		return this;
	}
